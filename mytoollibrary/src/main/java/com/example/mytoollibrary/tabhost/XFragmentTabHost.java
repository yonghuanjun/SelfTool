package com.example.mytoollibrary.tabhost;
...
import com.example.mytoollibrary.R;
...
public class XFragmentTabHost extends CustomTabHost {
    ...
    private void _init(Context context) {
        ...
        mContext = context;
        mTextActiveColor = ContextCompat.getColor(mContext, R.color.mainColor);
        mTextInactiveColor = ContextCompat.getColor(mContext, R.color.colorInactive);
        mFrontColor = ContextCompat.getColor(mContext, R.color.mainColor);
        mBehindColor = ContextCompat.getColor(mContext, R.color.mainColor);
        ...
        mViewActivePaddingTop = (int) getResources().getDimension(R.dimen.space_3);
        mViewInactivePaddingTop = (int) getResources().getDimension(R.dimen.space_17);
        mTabMode = TabMode.ClipDrawable;
    }
    ...
    public void addTabItem(TabItem item, Class<?> fragClass, Bundle bundle) {
        ...
        if(bundle.getInt("num")==3){
            home_num_tv.setVisibility(VISIBLE);
        }
     ...
     * @param isActivated 是否激活
     */
    TextView home_num_tv,tab_title;
    ...
    public TextView getMsgNum(int index ) {
        View view = mTabViews.get(index);
        home_num_tv = view.findViewById(R.id.home_num_tv);
        return home_num_tv;

    } public TextView getMsgTextNum(int index ) {
        View view = mTabViews.get(index);
